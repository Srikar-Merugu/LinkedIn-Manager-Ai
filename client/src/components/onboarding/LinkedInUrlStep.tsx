...
import { Linkedin, Loader2, AlertCircle, Link as LinkIcon } from 'lucide-react';

interface LinkedInUrlStepProps {
  onComplete: (data: { linkedinUrl: string }) => void;
  onSkip: () => void;
}

export function LinkedInUrlStep({ onComplete, onSkip }: LinkedInUrlStepProps) {
  const [url, setUrl] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);

  const validateUrl = (value: string): boolean => {
    return /linkedin\.com\/in\/[a-zA-Z0-9_-]+/.test(value);
  };
  ...
  const handleContinue = async () => {
    ...
    setLoading(true);
    onComplete({ linkedinUrl: url.trim() });
  };
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-surface-100 mb-2">Your LinkedIn Profile</h2>
        <p className="text-surface-400">We'll analyze your public profile to build your brand intelligence.</p>
      </motion.div>

      <div className="glass rounded-2xl p-6 mb-6">
        <label className="block text-sm font-medium text-surface-300 mb-2">LinkedIn Profile URL</label>
      ...
      <div className="glass rounded-2xl p-4 mb-6">
        <p className="text-xs text-surface-400">
          We analyze your public profile data including headline, experience, skills, and education.
