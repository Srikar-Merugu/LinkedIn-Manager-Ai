...
export default function LinkedInAnalysisPage() {
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      doc.setFontSize(12);
      doc.text('PersonaOS', margin, 40);

      doc.setFontSize(32);
      doc.setTextColor(255, 255, 255);
      doc.text('LinkedIn Analysis', margin, 70);

      doc.setFontSize(14);
      doc.setTextColor(150, 150, 180);
      doc.text('Professional Profile Report', margin, 82);

      doc.setFontSize(48);
      doc.setTextColor(100, 130, 255);
      doc.text(`${report.profileScore || 0}/100`, margin, 130);

      doc.setFontSize(16);
      doc.setTextColor(200, 200, 220);
      const label = (report.profileScore || 0) >= 80 ? 'Excellent' : (report.profileScore || 0) >= 60 ? 'Good' : (report.profileScore || 0) >= 40 ? 'Needs Improvement' : 'Getting Started';
      doc.text(label, margin, 145);

      doc.setFontSize(10);
      doc.setTextColor(120, 120, 150);
      doc.text(`Generated: ${report.generatedAt ? new Date(report.generatedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'N/A'}`, margin, 170);
      doc.text('Generated by PersonaOS - AI LinkedIn Consistency Coach', margin, 178);
      ...
      const summaryLines = doc.splitTextToSize(report.aiSummary || 'No summary available.', contentWidth);
      ...
      doc.setFontSize(18);
      doc.setTextColor(255, 255, 255);
      doc.text('Profile Health', margin, y);
      y += 12;

      report.profileHealth?.forEach((item) => {
        checkPage(10);
        const statusColor = item.status === 'strong' ? [80, 200, 120] :
          item.status === 'good' ? [100, 150, 255] :
          item.status === 'needs_improvement' ? [255, 180, 50] : [255, 80, 80];
        doc.setFontSize(9);
        doc.setTextColor(...statusColor as [number, number, number]);
        doc.text(`[${item.status.toUpperCase()}]`, margin, y);
        doc.setTextColor(200, 200, 220);
        doc.text(`${item.section}: ${item.details}`, margin + 35, y);
        y += 6;
      });
      y += 5;
      ...
      doc.setFontSize(18);
      doc.setTextColor(255, 255, 255);
      doc.text('Profile Strengths', margin, y);
      y += 12;
      ...
      report.strengths?.forEach((item) => {
        ...
        doc.text('+', margin, y);
        doc.setTextColor(255, 255, 255);
        doc.text(item.title, margin + 6, y);
        y += 5;
        doc.setTextColor(150, 150, 180);
      ...
      doc.setFontSize(18);
      doc.setTextColor(255, 255, 255);
      doc.text('Missing Sections', margin, y);
      y += 12;

      report.missingSections?.forEach((item) => {
        checkPage(12);
        const pColor = item.priority === 'high' ? [255, 100, 100] :
          item.priority === 'medium' ? [255, 180, 50] : [150, 150, 180];
        doc.setFontSize(8);
        doc.setTextColor(...pColor as [number, number, number]);
        doc.text(`[${item.priority.toUpperCase()}]`, margin, y);
        doc.setTextColor(255, 255, 255);
        doc.text(item.section, margin + 25, y);
        y += 5;
        doc.setTextColor(150, 150, 180);
      ...
      report.improvements?.forEach((item) => {
        ...
        doc.setTextColor(150, 150, 180);
        doc.text(`(${item.effort})`, margin + 25 + doc.getTextWidth(item.title) + 4, y);
        y += 5;
        doc.setTextColor(180, 180, 200);
        const descLines = doc.splitTextToSize(item.description, contentWidth - 10);
        doc.text(descLines, margin + 6, y);
        ...
        doc.setTextColor(80, 200, 120);
        doc.text(`Impact: ${item.impact}`, margin + 6, y);
        y += 6;
      });
      y += 3;
      ...
      doc.setFontSize(18);
      doc.setTextColor(255, 255, 255);
      doc.text('Content Opportunities', margin, y);
      y += 12;
      ...
      report.contentOpportunities?.forEach((item) => {
        ...
        doc.setFontSize(10);
        doc.setTextColor(100, 130, 255);
        doc.text(`${item.engagementScore}`, margin, y);
        doc.setTextColor(255, 255, 255);
        doc.text(item.topic, margin + 12, y);
        y += 5;
        doc.setFontSize(9);
        doc.setTextColor(150, 150, 180);
        const reasonLines = doc.splitTextToSize(item.reason, contentWidth - 10);
        ...
        doc.setTextColor(120, 120, 150);
        doc.text(`Pillar: ${item.pillar}`, margin + 6, y);
        y += 6;
      });
      ...
      doc.text('Generated by PersonaOS - AI LinkedIn Consistency Coach', margin, y);
      doc.text(new Date().toISOString(), pageWidth - margin - 40, y);

      const firstName = 'User';
      doc.save(`LinkedIn_Analysis_Report_${firstName}_${new Date().toISOString().split('T')[0]}.pdf`);
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleShare = async () => {
    ...
    const text = `My LinkedIn Profile Score: ${shareData.profileScore}/100 | ${shareData.strengths} strengths identified | ${shareData.missing} areas to improve — Analyzed with PersonaOS`;

    if (navigator.share) {
      try {
        await navigator.share({ title: 'LinkedIn Analysis - PersonaOS', text, url: window.location.href });
      } catch {}
    } else {
      try {
        ...
        <GlassCard>
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleExportPDF} disabled={exporting}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-brand-500/10 border border-brand-500/20 text-sm text-brand-400 hover:bg-brand-500/20 transition-all font-medium">
            ...
            <button onClick={handleShare}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/[0.03] border border-white/5 text-sm text-surface-300 hover:text-surface-100 hover:bg-white/[0.05] transition-all relative">
              <Share2 className="w-4 h-4" /> Share
              <AnimatePresence>
                {shareTooltip && (
                  <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
