  ...
  private addOpportunitiesForSkill(
    ...
    if (!map.has(id(`${skill}-teach-1`))) {
      map.set(id(`${skill}-teach-1`), {
        ...
        impact: isTechnical ? 80 : 70,
        authority: totalYears > 5 ? 80 : 70,
        engagementPotential: isTechnical ? 78 : 72,
        careerAlignment: seniority === 'senior' ? 85 : 75,
        overallScore: 0,
        effort: 'low',
      });
    }
    ...
    if (totalYears > 3 && !map.has(id(`${skill}-experience`))) {
      map.set(id(`${skill}-experience`), {
        ...
        impact: Math.min(70 + totalYears * 3, 90),
        authority: Math.min(75 + totalYears * 2, 90),
